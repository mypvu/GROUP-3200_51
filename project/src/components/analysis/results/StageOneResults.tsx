    ...
    <div className="mb-8">
        ...
        >
            ...
            {unknownCompound && (
                ...
                <td className="border p-3 text-blue-900">Unknown</td>
                <td className="border p-3">{unknownCompound.RF}</td>

                <td className="border p-3" data-hue={unknownCompound.DEV_254nm ?? ""}>
                    {unknownCompound.DEV_254nm}
                </td>
                <td className="border p-3" data-hue={unknownCompound.DEV_366nm ?? ""}>
                    {unknownCompound.DEV_366nm}
                </td>
                <td className="border p-3" data-hue={unknownCompound.VSNP_366nm ?? ""}>
                    {unknownCompound.VSNP_366nm}
                </td>
                <td className="border p-3" data-hue={unknownCompound.T ?? ""}>
                    {unknownCompound.T ?? ""}
                </td>

                {/* FI λ1–λ3 */}
                <td className="border p-3">{unknownCompound.FL_Peaks?.[0] ?? ""}</td>
                <td className="border p-3">{unknownCompound.FL_Peaks?.[1] ?? ""}</td>
                <td className="border p-3">{unknownCompound.FL_Peaks?.[2] ?? ""}</td>

                {/* UV-Vis λ1–λ3 */}
                <td className="border p-3">{unknownCompound.UV_Peaks?.[0] ?? ""}</td>
                <td className="border p-3">{unknownCompound.UV_Peaks?.[1] ?? ""}</td>
                <td className="border p-3">{unknownCompound.UV_Peaks?.[2] ?? ""}</td>
                </tr>
            )}
            ...
            {compounds.length === 0 ? (
                ...
                <tr className="odd:bg-gray-50" key={compound.id ?? compound.name ?? i}>
                    <td className="border p-3 font-semibold">{compound.name ?? compound.id}</td>
                    <td className="border p-3">{compound.RF}</td>

                    <td className="border p-3" data-hue={compound.DEV_254nm ?? ""}>
                    {compound.DEV_254nm}
                    </td>
                    <td className="border p-3" data-hue={compound.DEV_366nm ?? ""}>
                    {compound.DEV_366nm}
                    </td>
                    <td className="border p-3" data-hue={compound.VSNP_366nm ?? ""}>
                    {compound.VSNP_366nm}
                    </td>
                    <td className="border p-3" data-hue={compound.T ?? ""}>
                    {compound.T ?? ""}
                    </td>

                    {/* FI λ1–λ3 */}
                    <td className="border p-3">{compound.FL_Peaks?.[0] ?? ""}</td>
                    <td className="border p-3">{compound.FL_Peaks?.[1] ?? ""}</td>
                    <td className="border p-3">{compound.FL_Peaks?.[2] ?? ""}</td>

                    {/* UV-Vis λ1–λ3 */}
                    <td className="border p-3">{compound.UV_Peaks?.[0] ?? ""}</td>
                    <td className="border p-3">{compound.UV_Peaks?.[1] ?? ""}</td>
                    <td className="border p-3">{compound.UV_Peaks?.[2] ?? ""}</td>
                </tr>
                ))
            )}
            </tbody>
