...
class BasicFilterHelper {
    ...
    applyAllFilters(ranges: RangeParams, T?: boolean): BasicFilterHelper {
        this.res = this.data.filter((compound) => {
            // Only enforce db_label match if both sides are defined
            if (this.c?.db_label != null && compound.db_label != null &&
                this.c.db_label !== compound.db_label) {
                return false;
            }
